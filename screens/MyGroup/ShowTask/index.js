  ...
  useEffect (
    () => {
      if (task.responsibles) {
        _setGroupMembers0 (
          task.responsibles.slice (0, Math.ceil (task.responsibles.length / 2))
        );
        _setGroupMembers1 (
          task.responsibles.slice (
            Math.ceil (task.responsibles.length / 2),
      ...
      render: () => (
        <Button key="accepting" transparent onPress={handleOnToggleCompleted}>
          <MaterialCommunityIcons
            ...
            color={task.completed ? theme.DANGER_COLOR : theme.SUCCESS_COLOR}
            size={theme.ICON_SIZE_MEDIUM}
          />
        </Button>
      ),
  ...
  const FloatingUsers = () => {
    return (
      <Row style={styles.userRow}>
        <Col>
          {group_members0.map (
            ({taskCompleted, atendee: {user: member}}, i) => {
