      ...
      <HeaderComponent group={group} taskName={task.taskName} description={task.description}  goBack={goBack}/>
        {
          ...
          <View style={styles.bodyContainer}>
            <NoResults lottieProps={{autoSize: true, style:{width: wp(30)}}} animationName="user-scanning" primaryText="¡No se ha encontrado ningún usuario!" primaryTextStyles={{color: 'white'}}/>
          </View>
        }
        <Button
          primary
          full
          onPress={() => {
            dispatch ({type: 'events/COMPLETE_TASK', payload: {task, goBack, navigate}});
            // navigate ('Groups');
          }}
...
MyTask.navigationOptions = ({navigation}) => {
  return {
    // title: '',
    // header: null,
    // headerLeft: (
